...
import { clearTempUserData, setTempUserData } from "../../../features/user/otpSlice";
...
function SubmitOtp() {
    ...
    const tempUser = useSelector((data: RootState) => data.otp.tempUserData);
    ...
    useEffect(() => {
        ...
        let interval = setInterval(() => {
            let dt = Date.now();
            let diff = Math.floor((dt - parseInt(tempUser?.time || "0")) / 1000);
            let second = 30 - diff;
            if (second < 0) {
    ...
    const handleOtp = async (event: React.FormEvent) => {
        ...
        const res = await submitOtp({ ...tempUser, otp: data });
    ...
    const resendOtp = async () => {
        if (otp > 0) {
            ...
            setLoading(true);
            setData('')
            const res = await resendOTP(tempUser);
            console.log(res?.user);
            if (res?.success) {
                dispatch(setTempUserData(res?.user));
                setLoading(false);
            }
        }
    };
