...
function GoogleIcon() {
     ...
     const handleGoogleSignup = async () => {
          try {
               ...
               const photo = result.user.photoURL as string;
               const user = {
                    name: result.user.displayName,
                    email: result.user.email,
                    image: result.user.photoURL,
                    //     image:photo.replace("=s96-c", "=s288-c-no")
               };

               const res = await axiosInstance.post("/google/auth", user);

               dispatch(setUserCreadential(res.data.user));
          } catch (error) {
     ...
     return (
          <div onClick={handleGoogleSignup} className="rounded w-[25px] h-[25px]  ml-[20px]">
               <img
