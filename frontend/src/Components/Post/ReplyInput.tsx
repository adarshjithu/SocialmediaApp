...
import { replyComment } from "../../Services/apiService/postServices";
...
function ReplyInput({ comment }: {comment:CommentInterface}) {
    ...
    const handleAddComment = async () => {
        if (newComment === "") {
            ...
            setLoading(true);
            const res = await replyComment(newComment, comment?._id);
            setLoading(false);
            console.log(res?.data.result.replies);
