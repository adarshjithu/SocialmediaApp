...
function StatusBar() {
    ...
    const [myStory, setMyStory] = useState<Record<string, any>>({});
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            dispatch(getAllStatusData(res?.data?.result?.allStory));
            setMyStory(res?.data?.result?.myStory);
            setLoading(false);
        };
        fetchData();
    }, []);
            ...
            <div className="mr-6">
                <div className=" status-outer w-[60px] h-[60px] flex justify-center items-center rounded-[100%]">
                    <div className="w-[55px] h-[55px] rounded-[100%] bg-white relative ">
                        <img onClick={()=>dispatch(viewStoryBar({ view: true, stories: myStory }))} src={myStory?.stories ? myStory?.stories[0]?.image : ""} alt="" className="rounded-[100%] w-full h-full" />
                        <span
                            onClick={() => {
                                dispatch(isStatusUpload(true));
                            }}
                            className="cursor-pointer text-[40px] h-[30px] rounded-[100%] absolute right-[-10px] bottom-[-2px] bg-[green] flex jusfify-center items-center text-[white] "
                        >
                            +
                        </span>
                    </div>
                </div>
                <span style={{ color: `${theme.theme == "black" ? "white" : "black"}` }}>My Story</span>
            </div>
                    ...
                    <div key={data._id} className="mr-6" onClick={() => dispatch(viewStoryBar({ view: true, stories: data }))}>
                        <div className=" status-outer w-[60px] h-[60px] flex justify-center items-center rounded-[100%]">
                            <div className="w-[55px] h-[55px] flex justify-center items-center rounded-[100%] bg-white">
                                <img src={data.stories[0].image} alt="" className="w-full h-full rounded-[100%]"  />
                            </div>
                        </div>
